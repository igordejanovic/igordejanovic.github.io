...
#+begin_note
Obratiti pažnju da je, kod definisanja korisničkih klasa, neophodno obezbediti
ovaj parametar kao prvi parametar konstruktora kod svih klasa koje predstavljaju
dete u vezi roditelj-dete jer će textX pozivati konstruktor uz prosleđivanje
reference na roditeljski objekat prilikom parsiranja.
#+end_note
...
budu pravi objekti a ne samo ključne reči, kreiraćemo nove objekte i
registrovati nad metamodelom. Naravno, da bi mogli instancirati klase u Python-u
potrebno je da ih registrujemo kao korisničke klase (slajd [[Korisničke klase]]).
...
*** Konfiguracija parsera - /case-sensitivity/
...
*** Konfiguracija parsera - /whitespaces/
...
*** Konfiguracija parsera - /keywords/
...
xdot^{1}, ZGRViewer^{2}) ili se, upotrebom /dot/ alata, fajl može transformisati u
neki od bitmapiranih ili vektorskih grafičkih formata (npr. PNG, SVG, JPG, PDF).
...
Vizualizacija može da se obavi i upotrebom [[#textx-komanda][=textx= komande]] opisane u nastavku.
...
** RREL primer - gramatika/metamodel
Videti [[https://github.com/textX/textX/tree/master/tests/functional/test_scoping][textX testove]].
...
** Registracija jezika
- Instancirati =LanguageDesc= klasu gde kao parametre navodimo: jedinstveni naziv
  jezika, fajl obrazac/ekstenziju za modele na datom jeziku, opis i funkciju
  (tačnije Python /callable/) koji vrši kreiranje i konfiguraciju metamodela.
...
- Deklarativan način registracije putem =setup.py=, odnosno =setup.cfg=. Koristimo
  ulazne tačke (eng. /entry points/), standardan mehanizam Python =setuptools=
  paketa:
...
Ulazna tačka se zove =textx_languages= - lista stringova oblika "<ime jezika> =
<putanja_do_LanguageDesc_instance>". U ovom primeru instanca =LanguageDesc= se
nalazi u paketu =entity.metamodel=. Varijable tj. referenca se zove =entity_lang=.
...
- Dekorator =language= - parametri su naziv jezika i ekstenzija fajlova model.
  /Docstring/ se koristi kao opis.
...
- Koristimo instancu =GeneratorDesc=. Pri instanciranju navodimo: naziv jezika,
  naziv ciljne tehnologije, opis i na kraju funkciju koja vrši generisanje.
  Funkcija mora biti sledećeg oblika:
...
- Dekorator =generator= - parametri su naziv jezika i naziv ciljne platforme. Opis
  generatora se navodi u /docstring/-u generator funkcije.
...
- Registracija se vrši u =setup.cfg= (ili =setup.py=) na isti način kao i jezik s
  tim što se ulazna tačka naziva =textx_generators=:
...
* Kreiranje inicijalnog projekta - /scaffolding/
...
Ova komanda pokreće generator koji postavlja par pitanja^{1} i zatim kreira
projekat u zadatom folderu. Dobra praksa je da se zatim projekat instalira u
tekuće radno okruženje u razvojnom modu sa:
...
#+attr_html: :class footer :style clear: both;
1. Odgovori na pitanja se keširaju i biće podrazumevana pri narednom pokretanju
   generatora.
...
** PyFlies - jezik za psihološke testove
- https://pyflies.github.io/pyflies/latest/
- https://github.com/pyflies
- [[https://www.youtube.com/playlist?list=PLOgHC04drILVjnXQTFEL7sJDyfKMR21Vg][Video tutorials]]
...
- [[https://textx.github.io/textX/][textX dokumentacija]]
