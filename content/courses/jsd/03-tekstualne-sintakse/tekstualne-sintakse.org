...
gde =else= pripada spoljnem =if= iskazu, ili kao:
#+begin_src
if a then (if b then s else s2)
#+end_src

gde =else= pripada unutrašnjem =if= iskazu.

- Rešavaju se dodavanjem pravila prioriteta ili dodavanjem konteksta kojim se
  izbegava višeznačnost. Na primer, za kôd =if-else= klauzule može se dodati
  ključna reč =endif=.
