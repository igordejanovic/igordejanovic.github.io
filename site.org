...
*** System mode
#+begin_src conf
set $mode_system System (l)ock, (e)lout, (s)usp, (r)eboot, (u)mount, (Shift+s) shutdown
mode "$mode_system" {
    ...
    bindsym u exec $umount_all; mode "default"
...
//      "idle_inhibitor",
  ...
  "network": {
      ...
      "interval": 3,
      "format-wifi": "{essid:.8} ({signalStrength}%,{bandwidthDownBits:>3}|{bandwidthUpBits:>3}) ",
      "format-ethernet": "{ifname}: {bandwidthDownBits:>3}|{bandwidthUpBits:>3} ",
...
***** Cycle color schemes - [[https://github.com/toggle-corp/alacritty-colorscheme/pull/44][not working currently]] due to move to TOML
Make a keyboard shortcut for easier [[#color-schemes][cycling of color scheme]]:
#+begin_src toml
[[keyboard.bindings]]
key = "F12"

[keyboard.bindings.command]
args = ["-V", "toggle", "wombat.yaml", "papercolor_light.yaml"]
program = "alacritty-colorscheme"
...
:END:
#+begin_quote
Warning: this is not working at the moment as alacrity has migrated to TOML
format.
#+end_quote
...
Toggle by (see above for keybinding in alacritty config):
#+begin_src sh
