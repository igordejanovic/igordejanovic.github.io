...
** [[https://textx.github.io/igordejanovic/parglare/][parglare]]
...
Read more information [[./parglare/][here]] or see the project on [[https://github.com/igordejanovic/parglare/][GitHub]].
...
:END:
** TODO Deploying Hugo site on GitHub Pages :github:hugo:
There is a [[https://github.com/benmatselby/hugo-deploy-gh-pages][GitHub Action to build and deploy a Hugo site to GitHub Pages]]. I made
a fork that I'll be using just to be sure what I'm running and to introduce
additional changes if needed. Following the docs, make a
=.github/workflow/deploy-site.yaml= with the following content (adapt as needed):
#+begin_src yaml
...
jobs:
  build:
    ...
    - name: Hugo Deploy GitHub Pages
      uses: igordejanovic/hugo-deploy-gh-pages@master
      env:
        HUGO_VERSION: 0.105.0
        TARGET_REPO: igordejanovic/igordejanovic.github.io
        TARGET_BRANCH: master
        TOKEN: ${{ secrets.TOKEN }}
        CNAME: www.igordejanovic.net
#+end_src

- The best approach is to use =HUGO_VERSION= the same as your local. Check local
  version by running =hugo version=.
- =with: submodules= is needed if site uses resources as [[https://git-scm.com/book/en/v2/Git-Tools-Submodules][git submodules]] (e.g. a
  theme checked out as a submodule).
- =secrets.TOKEN= is a repo secret, named =TOKEN= (can be called differently),
  produced from GitHub [[https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token][/Personal Access Token/ (PAT)]]. PAT is a token for
  accessing GitHub API using your credentials. Treat it as a password.
- /Not working at the moment/: GitHub offers fine-grained PATs, which can be
  restricted to access only a particular repo with a particular set of access
  rights (/currently in Beta and it seems that push is not supported/).
  1. Create it by going to =your profile->Settings->Developer settings->Personal
     access tokens->Fine-grained tokens=
  2. Choose expiration. Remember, you will have to renew it when expired.
  3. Choose access to only site repo.
  4. Choose these set of rights: commit, push, read secrets.
  5. Store PAT in a safe place (e.g. password manager). It will be needed for
     creating repo secret.
- /Use this for now/: PAT *classic* are tokens that enable access to all repos, thus
  they should be replaced ASAP with fine-grained PATs above.
  1. Create it by going to =your profile->Settings->Developer settings->Personal
     access tokens->Tokens (classic)=
  2. Type in note, expiration and enable =repo= scope. This gives full access to
     all repos to anyone who has the token!
- Create repo secret to be used as a =TOKEN= environment variable from the =yaml=
  workflow config above.
  - Go to site repo =Settings->Security-Secrets->Actions=.
  - Click =New repository secret= button, give it a name (e.g. =TOKEN=) and paste
    =PAT= as a value.
  - Remember, GitHub action code will have credentials you gave while creating
    =PAT= in the previous step. So be sure to trust the action you will be
    running. The best is to fork the action, do the code review to be sure what
    the action will be doing, and use your fork. From time to time pull upstream
    changes and review them.
- Now, whenever you push to =master= branch the workflow will run and it will
  clone, build and deploy your site.

*** Troubleshooting
**** Error building site, process: readAndProcessContent,  No such File or Directory, lstat...
This error is reported if there are symbolic links in the content folder with
unexisting targets. See [[https://discourse.gohugo.io/t/static-directory-error-decorate-lstat-no-such-file-or-directory/33317][here]]. To find all symbolic links do:
#+begin_src sh
find -type l .
#+end_src

**** fatal: detected dubious ownership in repository at '/github/workspace/public'
Reported when =public= folder in the git repo is not empty. No need to keep it in
the repo as it is built by Hugo during deploy. Remove it and add it to
=.gitignore=.

**** fatal: credential url cannot be parsed: =https://***...=
This happens if you have newline at the end of PAT for secret token.

**** Testing locally
As per docs. Clone Hugo action repo and build the docker image:
#+begin_src sh
docker build --pull --rm -f "Dockerfile" -t hugodeployghpages:latest
#+end_src

Now, you can run the docker locally (=GIT_TRACE= and =GIT_CURL_VERBOSE= are used for
verbose git commands debugging).
#+begin_src
docker run --rm \
  -eHUGO_EXTENDED=true \
  -eGIT_TRACE=true \
  -eGIT_CURL_VERBOSE=true \
-eGITHUB_TOKEN=<paste token here> \
  -eGITHUB_ACTOR=igordejanovic \
  -eTARGET_REPO=igordejanovic/igordejanovic.github.io \
  -v "$(pwd)":/site/ \
  --workdir /site \
  hugodeployghpages
#+end_src
**** Only the last commit visible!
- Create =.nojekyll= file:
  #+begin_src sh
touch .nojekyll
  #+end_src
  This will prevent Jekyll to run.
- Change hugo action to deploy =public= folder to the =gh-pages= branch. That branch
  can be git orphan branch recreated from scratch with each deploy. This will
  keep repo tidy.

** DONE Debugging Elisp :emacs:programming:
CLOSED: [2022-12-31 Sat 13:36]
:PROPERTIES:
...
my FLOSS projects are described [[#projects][here]]. Check it out at [[https://github.com/%7B%7B%20site.github%20%7D%7D][github]].
